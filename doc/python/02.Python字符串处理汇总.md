...
# 定义要执行的检查（函数名，说明，需要测试的字符串键列表）
checks = [
    (is_num, "是否为纯数字", ["s6", "s7"]),
    (is_chinese, "是否为纯汉字", ["s5", "s7"]),
    (is_letter, "是否为纯字母", ["s1", "s2"]),
    (is_Bletter, "是否为纯大写字母", ["s4", "s2"]),
    (is_Sletter, "是否为纯小写字母", ["s4", "s2"]),
    (is_num_letter, "是否只包含字母/数字/字母数字混合", ["s1", "s4"]),
]

# 执行测试并打印结果
for func, desc, keys in checks:
    print(desc)
    for key in keys:
...
1. **使用可变参数**：将固定的四个参数改为 `*keywords`，调用更灵活，便于扩展。
2. **统一逻辑**：每个关键词都明确输出“包含了”或“不包含了”，避免原来只对 `bh02` 有 `else` 的不一致行为。
3. **避免内置名冲突**：将变量名 `str` 改为 `text`。
4. **简化条件判断**：使用 `for` 循环遍历所有关键词，代码更简洁易读。
5. **利用 f-string**：让打印语句更清晰。

```python
def baohan(text, *keywords):
    for kw in keywords:
        if kw in text:
            print(f"包含了：{kw}")
        else:
