...
（1）先新建源文件：文件 -> 新建 -> 源代码![](https://i-blog.csdnimg.cn/blog_migrate/434e4b8820b98a69fa29a3a3db88d66f.jpeg)
...
![](https://i-blog.csdnimg.cn/blog_migrate/b1bd2489e0ec66ff9823a99d07b46832.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/6b90b2a9c9461f42a5de374047597ace.png)
...
#include <stdio.h>
//
// Created by Administrator on 25-6-16.
//
int main() {
    // 问题1：在屏幕上打印一个单引号，怎么做？
    printf("%c\n",'\'');
...
![](https://i-blog.csdnimg.cn/blog_migrate/ad8e08929257609d526889d179de1194.png)
...
case语句就像一个入口，如果没遇到 break，程序将会一直执行。这里的入口位置是任意的。

当我修改day的值以后，对于此时的day，入口发生改变了。
...
当我再次修改变量day的值为8，且加default语句结果如上。因为上面的 case 语句中没有对应的入口，只能通过 default 进入 switch 语句。
...
![](https://i-blog.csdnimg.cn/blog_migrate/888c6d959700e53bf7100fe609df33ba.png)
...
计数器的位置同样会影响打印结果。 可以发现，while 循环只不过是把条件判断留在了括号中，变量初始化和计数器放在了其他地方。
...
1. switch表达式必须是整型或枚举类型
2. case标签必须是常量表达式
3. break语句用于退出switch结构，没有break会继续执行下一个case
4. default子句是可选的，用于处理不匹配任何case的情况

switch语句相比if-else链在多个条件判断时通常更清晰、效率更高。

#### 7.3 do...while 语句

do...while是C语言中的一种循环控制语句，它与while循环类似，但有一个关键区别：do...while循环会先执行循环体，然后再检查循环条件。
...
>
> - 结尾有分号：while(条件)后面必须有一个分号
>
> - 适合先执行后判断的场景：当需要至少执行一次循环体时使用
...
>
> - 避免无限循环：确保循环条件最终会变为假
> - 可以在循环体内使用`break`提前退出循环
> - 可以使用`continue`跳过当前迭代，直接进入下一次条件判断
>
> 希望这个解释对您理解C语言的`do...while`语句有所帮助！
...
函数是C语言编程中的基本构建块，它是一组一起执行任务的语句，可以接收输入参数并返回一个值。
...
1. **返回类型**：函数返回值的数据类型，如`int`、`float`、`void`等
2. **函数名**：函数的标识符
3. **参数列表**：函数接收的输入参数，可以为空
4. **函数体**：包含函数执行的语句
5. **return语句**：返回函数结果（void函数可以省略）
...
运行以上代码，会有一个光标在闪动，输入3回车。代码中打印了三次，不同的是第二次，添加了一个换行符。其中输入语句中最后的是 &（取地址），现在只要记住输入要这个符号即可。
...
运行上述程序，输入 6699 回车，且不论打印的数字位置如何，它们却只有 2 位。

原因是：可以把输入输出语句中的双引号以及它里面的东西翻译成 “格式”，输入输出就是按格式走的，是强制的。

这个%多少d，“多少” 代表宽度，你看即使我输入4个数字，那它也只读取前两个数字。下面两个数字位置不同的原因是：打印函数的宽度正数靠右，负数靠左。如果没有规定宽度，那么也就没有绝对的靠右靠左对齐可言，不过从左到右习惯来讲，不规定宽度默认是靠左的。请注意对齐是在有移动空间的前提下才会体现出来。就像后面两个打印函数，长度（绝对值）都是4，是大于读取的2位数字的长度的。
...
在这里,"arr[5]" 内的数字可以不用写，编译器会根据元素个数来改变它
...
这行代码 `int size = sizeof(arr) / sizeof(arr[0]);` 的作用是 **计算数组 `arr` 的元素个数**，其中：

- `sizeof(arr)`：返回整个数组 `arr` 占用的内存字节数。
- `sizeof(arr[0])`：返回数组第一个元素 `arr[0]` 占用的内存字节数（即单个元素的大小）。
- `sizeof(arr) / sizeof(arr[0])`：用 **数组总大小** 除以 **单个元素大小**，得到 **数组的元素个数**。
...
![](https://i-blog.csdnimg.cn/blog_migrate/07806f64d70144f74b1b8f548d901832.png)

算数操作符和赋值操作符的前五个相信大家都知道，这里提一下位操作符'&'，当我们每次使用 scanf 函数时，总是要在变量名前加上 &，& 变量 a 的意思是取变量 a 的地址，这是 & 符号的作用之一。

![](https://i-blog.csdnimg.cn/blog_migrate/b082f6997896e923ff8be93801d91ce1.png)         对于上面的单目操作符和关系操作符，我们可能对解引用操作符不太熟悉（*），它和刚才提到的 & 符号取变量地址是对应的，例如：*a = &a，它们是完全等价的。不太明白没关系，这里以后会重点讲。

![](https://i-blog.csdnimg.cn/blog_migrate/802fa36a8abfbcb8cbda2ea278d15ff2.png)         这个条件操作符我们总是不容易想到，不经常使用，但有时它能有种四两拨千斤的效果，能简化不少。

        这个操作符的意思是：当表达式一的结果为真，整个表达式结果就为表达式 2，否则结果为表达式 3。例如：

![](https://i-blog.csdnimg.cn/blog_migrate/d1f17a8589cac806e5b99c43182da5f9.png)

        逗号表达式的意思就是一个语句由 N 个被逗号分隔的子语句（实际上不能称之为语句）组成，那么这个语句的结果就为最后一个子语句的结果。例如：

 ![](https://i-blog.csdnimg.cn/blog_migrate/b41d69eafd552d0ee5720ba415015740.png)

9. 常见关键字
--------

![](https://i-blog.csdnimg.cn/blog_migrate/fedbca99b75591359f11132686048b49.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/174ddce7e0d647e1af1d060155b88eb9.png)

![](https://i-blog.csdnimg.cn/blog_migrate/99ca6457d862e8df8e19024167649e72.png)

        在这里唯一的不同是在 a = 2 后加了 break，它的作用是阻止语句往下执行。也就是说，switch 语句要么从入口一直执行完毕，要么从入口开始执行，直到遇到 break 终止。

 循环例子如下：

![](https://i-blog.csdnimg.cn/blog_migrate/52bcbca9a898c262f243fc2e789acab3.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/ad1f5881b3a6e0a97982bd1a870119a8.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/a80bc171766afd58789e331015fdb95e.png)

![](https://i-blog.csdnimg.cn/blog_migrate/2cb58501f521733883389ed33332a524.png)
...
 ![](https://i-blog.csdnimg.cn/blog_migrate/38cc84cec081e6d95121ce10c1998fed.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/69d170159cf4d672fcbc45739168fdca.png)
