...
# 1. 获取当前目录的名称（替换原有固定的java_var）
# os.getcwd() 获取当前工作目录的完整路径，os.path.basename() 提取路径最后的目录名
...
for file_name in os.listdir("."):
    ...
    if file_name.endswith(".md") and file_name not in exclude_files:
        ...
        # 5. 拼接指定格式的侧边栏条目（使用当前目录名替代原有java_var）
        sidebar_item = f"* [{file_title}](doc/{current_dir_name}/{file_title})"
...
if __name__ == "__main__":
    ...
    print(f"当前使用的目录名：{current_dir_name}")  # 新增：打印当前目录名，方便验证