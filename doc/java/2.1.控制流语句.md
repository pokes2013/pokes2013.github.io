...
顺序结构执行流程图:
![](./images/e7d5262d8ef7fb65cb164fdf03e06d49.png)
...
![](./images/a3216729cc9a684d436c87f05d13db53.png)
...
4. 继续执行后面的语句内容
   ![](./images/2bc16c82c73829b87a9a073a479e3588.png)
   示例:
...
5. 如果没有任何关系表达式为 true，就执行语句体 n+1。
   ![](./images/c26069942fe9b48137b41a026dccc75f.png)
...
* 最后，如果所有的 case 都和表达式的值不匹配，就会执行 default 语句体部分，然后程序结束掉。
  ![](./images/45a9d827b561130f3e45652060850195.png)
...
> 循环：循环语句可以在满足循环条件的情况下，反复执行某一段代码，这段被重复执行的代码被称为循环体语句，当反复 执行这个循环体时，需要在合适的时候把循环判断条件修改为 false，从而结束循环，否则循环将一直执行下去，形成死循环。
...
解释：什么是水仙花数？水仙花数，指的是一个三位数，个位、十位、百位的数字立方和等于原数。例如:`153 3*3*3 + 5*5*5 + 1*1*1 = 153`
...
概述：Random 类似 Scanner，也是 Java 提供好的 API，内部提供了产生随机数的功能。API 后续课程详细讲解，现在可以简单理解为 Java 已经写好的代码
